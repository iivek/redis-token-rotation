...
from token_refresh.decorators import decode_output_to_string, decode_args_to_string
...
async def test_decode_from_bytes_decorator():
    ...
    result_bytes_args = await dummy_function(b'Hello', b'World', key1=b'Value1', key2=b'Value2')
    expected_bytes_args = ['Hello', 'World', {'key1': 'Value1', 'key2': 'Value2'}]
    assert result_bytes_args == expected_bytes_args

    # Test case with mixed arguments (bytes and non-bytes)
    result_mixed_args = await dummy_function(b'Hello', 'World', key1=b'Value1', key2='Value2')
    expected_mixed_args = ['Hello', 'World', {'key1': 'Value1', 'key2': 'Value2'}]
    assert result_mixed_args == expected_mixed_args

    # Test case with non-bytes arguments
    result_no_bytes = await dummy_function('Hello', 'World', key1='Value1', key2='Value2')
    expected_no_bytes = ['Hello', 'World', {'key1': 'Value1', 'key2': 'Value2'}]
